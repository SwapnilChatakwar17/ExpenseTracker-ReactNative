...
import { dateFormat, lastDays } from "../util/dateFormat";
...
const RecentExpense = (props) => {
  ...
  const recentDate = lastDays(new Date(), 7).toString();

  const expenseData = ExpenseCtx.expense.filter((expense) => {
    // console.log(expense);
    return dateFormat(expense.date).toString() > recentDate;
  });
  ...
  useLayoutEffect(() => {
    setTotalExpense(() => {
      let sum = 0;
      sum = expenseData.reduce((temp, exp) => temp + Number(exp.amount), 0);
      return sum.toString();
    });
  }, [ExpenseCtx]);
    ...
    <View style={MyStyle.screen}>
      <View style={styles.card}>
        <View style={styles.detailContainer}>
          <Text style={styles.detailTitleTxt}>Total 7 Days Expense</Text>
        </View>
      ...
      <View style={styles.list}>
        <FlatList data={expenseData} renderItem={expenseHandler} />
      </View>
    </View>
  );
};
...
const styles = StyleSheet.create({
  card: {
    ...
    maxHeight: Dimensions.get("window").width * 0.20,
    margin: 10,
    paddingHorizontal: 5,
    paddingVertical: 5,
  },
